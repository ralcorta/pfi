...
resource "aws_security_group" "mirror_target" {
  ...
  ingress {
    ...
    description = "Tráfico mirroring TCP"
  }
  ...
  ingress {
    ...
    protocol    = "udp"
    cidr_blocks = ["0.0.0.0/0"]
    description = "Tráfico mirroring UDP"
  }
...
resource "aws_security_group" "sensor" {
  ...
  ingress {
    ...
    description     = "Tráfico desde mirror target"
  }
...
resource "aws_security_group" "ecs_mirror" {
  ...
  ingress {
    ...
    protocol        = "tcp"
    security_groups = [aws_security_group.mirror_target.id]
    description     = "Tráfico mirroring desde ENI"
  }
...
resource "aws_ecs_task_definition" "sensor_task" {
  ...
  cpu                      = "256" # 0.25 vCPU
  memory                   = "512" # 512 MB
  # Sin execution_role_arn - usa LabRole por defecto
  # Sin task_role_arn - usa LabRole por defecto
